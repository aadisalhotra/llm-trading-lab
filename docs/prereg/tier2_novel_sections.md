...
Mechanical count as of the **2026-09-15** landing: **14** bracketed markers in the
...
**2026-09-15 landing: 14, unchanged.** The four wash-trade/barrier texts (Gate
4's amended taxonomy, cross-book submission-order parity, the barrier
mechanism, and BELOW_VENUE_MINIMUM) carry no bracketed markers — they are
settled registrations sourced from the 2026-08-29 integration spike and the
2026-09-15 RTH probe, not open branches awaiting a decision.
...
Gate 4 — Execution integrity (live phase). For live-phase segments, each model-month's execution success rate is the fraction of decision cycles whose intended orders were (i) successfully submitted to the venue and (ii) resolved to a terminal, broker-reconciled state — filled, partially filled, rejected by a legitimate market constraint (halt, liquidity, buying-power), or rejected under WASH_REJECT_POST_BARRIER (the account-wide wash-trade constraint firing after the P1 cross-book barrier has already serialized the contended submission; registered expectation ≈0, since the barrier's release condition is verified against both venue terminal states — a nonzero rate is a barrier-implementation signal, not a routine venue collision) — with the book reconciled to broker-authoritative positions and cash. A cycle fails execution when orders cannot be submitted (venue/API/auth failure), order status is unresolved, or post-cycle reconciliation diverges from broker state; a reconciliation divergence is additionally a book-integrity event (Gate 2). Legitimate market rejections are execution-successful — the linkage worked; they are logged as execution-constraint events per the registered censoring principle. Threshold: ≥0.80 per model-segment, uniform with the decision-completeness gate. Relationship to the inclusion gates: Gate 4 joins Gates 1–3 for live-phase estimability; it is inapplicable to paper-phase segments (simulated fills; trivially 1.0) and is registered as live-phase-scoped. A segment failing only Gate 4 is classified decision-estimable (execution-impaired): decision-level estimands (RQ1 concordance, RQ6) remain estimable with disclosure, while outcome-dependent estimands (RQ2/RQ3 outcome legs, RQ5's realized paths, all performance figures) are non-estimable for the segment — decisions were observed; their portfolio consequences were not. Correlation disclosure: execution failures are typically venue-wide and simultaneous across all six books (a broker outage is one event, not six); per-model rates are registered but their cross-book correlation is disclosed, and any venue-wide failure day is an incident-handling event regardless of monthly rates. October obligation: the validation month must demonstrate the execution-success measurement itself — the metric computes, the reconciliation check runs, and a deliberately induced submission failure is correctly classified (a test of the gate, not just of the venue).

**Landing note on the Gate 4 amendment (2026-09-15).** The paragraph above amends the committed Gate 4 text landed 2026-08-13/14, adding WASH_REJECT_POST_BARRIER as a third enumerated success class rather than registering it as a parallel statement. Before this landing, a wash-trade rejection was deliberately NOT filed with legitimate rejections (`src/execution/executor.py`, `CONSTRAINT_WASH_TRADE_BLOCK`: "an artifact of our account structure, not a market constraint") and its Gate 4 class was an open hub/Research question pending the cross-book barrier's design. This landing resolves that question for the POST-BARRIER case only: once P1's barrier serializes a contended submission, a wash rejection that still occurs is a structural, expected-near-zero artifact of the barrier's own mechanics rather than the routine, un-mediated collision the un-barriered account structure produces today. Until P1 ships, every wash-trade rejection observed in the live pipeline remains the un-mediated case and is NOT filed under this class.

Cross-book submission-order parity (registered, P1 barrier component). When two or more books submit opposite-side orders on the same symbol within one 30-minute cycle, which side reaches the venue first must be a registered, deterministic rule rather than an artifact of submission-race incidence — an unregistered tie-break would be an effective-parity defect across books. Priority order is a deterministic rotation: the six books are ordered by a base permutation rotated by t mod 6, where t is the cycle's ordinal tick index, so priority visits each book once per six-cycle window, reproducibly from t alone, with no randomness and no dependence on wall-clock submission time. Within a single book's own order batch for a cycle, sell-side orders (SELL, SHORT — both submit as a broker `sell`, per `src/execution/executor.py`'s `_do_sell`/`_do_short`) are submitted before buy-side orders (BUY, COVER — both submit as a broker `buy`, per `_do_buy`/`_do_cover`). Both rules are registered as carrying no economic content: they are deterministic tie-breaks for a structural account-level constraint, not a signal about position sizing, direction preference, or urgency, and are disclosed as such so neither is read as a registered trading rule.

Cross-book submission barrier (P1, registered mechanism). Resolves the account-wide wash-trade collision (`CONSTRAINT_WASH_TRADE_BLOCK`; ~7.7% of trading cycles carry a same-symbol opposite-side collision, long-only, measured over the Phase A record) by serializing only the contended portion of a cycle's submissions rather than submitting the whole cycle at once. Three composition rules:

1. **Scope** — the barrier engages only for a contended symbol: a ticker on which two or more books submit opposite-side orders (one side BUY/COVER, the other SELL/SHORT) within the same cycle. An uncontended order is unaffected and submits in the cycle's ordinary first segment.
2. **Ordering** — among contended books on a symbol, the submission-order-parity rule above (rotation by t mod 6, sells-first within book) determines which side submits first; the other side's order is held, not submitted, until the barrier releases it.
3. **Release condition** — the barrier releases the held order the instant the first order reaches a terminal broker state, FILLED or CANCELLED. Both terminal states are verified to clear the venue's wash-trade block (CANCELLED: 2026-08-29 integration spike; FILLED: 2026-09-15 RTH probe, item 3). An OPEN order — submitted, unresolved — does not release the barrier; item 3b (same run) confirms the block still fires against a same-symbol opposite order that remains open.

Cost: at most one extra segment per cycle — a symbol has two sides, and multiple contended symbols in one cycle resolve in parallel with each other, not serially. Against the registered concurrent-submission analysis (submit-all-then-poll-all: 1 segment = 5 minutes, 2 segments = 10 minutes, inside the 30-minute cycle and the p99/max order-count envelope of 7 orders/35 minutes and 9 orders/45 minutes that motivated moving off strictly sequential per-book submission), the barrier's added segment leaves 20 minutes of headroom. The registered 5-minute fill deadline is unchanged.
...
Below-venue-minimum execution constraint (registered). The venue enforces a $1 minimum order cost basis (verified 2026-08-29); fractional position sizing can produce a computed order smaller than that floor, and the pipeline catches it pre-submission rather than collecting it as a venue rejection. This is a legitimate venue limit and is treated as an execution-constraint event — G-EXEC-successful, the linkage worked — by the same reasoning as the settled-funds block above: the order is correctly never sent, and the blocked intention enters no behavioral outcome metric under the registered censoring principle. `CONSTRAINT_BELOW_VENUE_MINIMUM` (`src/execution/executor.py`) is this event's marker; it supersedes the interim "UNREGISTERED CLASSIFICATION" code comment at this landing (2026-09-15).
